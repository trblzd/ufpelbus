...
// Paradas que têm conflito de localização
...
export const entrarNaViagem = async (
  ...
  // Verifica se usuário já está em outra viagem
  ...
  await runTransaction(db, async (transaction) => {
    ...
    // 🔧 CORREÇÃO: Se não há rastreador ativo, este usuário vira rastreador
    ...
    // Se tem rastreador, decide papel baseado na distância do destino
  ...
  if (paradaDetectada && indiceDetectado !== -1) {
    try {
      await runTransaction(db, async (transaction) => {
        ...
        // 🔧 CORREÇÃO: Permite saltos de paradas
        if (indiceDetectado > indiceNoBanco) {
          const paradasPuladas = indiceDetectado - indiceNoBanco - 1;
          transaction.update(viagemRef, {
          ...
          if (paradasPuladas > 0) {
            console.log(
              `[Parada] Pulou ${paradasPuladas} parada(s): ${indiceNoBanco} → ${indiceDetectado} (${paradaDetectada})`,
            );
          } else {
            console.log(
              `[Parada] Avanço normal: ${indiceNoBanco} → ${indiceDetectado} (${paradaDetectada})`,
            );
          }
        }
      });
